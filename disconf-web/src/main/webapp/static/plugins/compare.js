...
$.fn.extend({
    CompareTxt: function (dom1, dom2) {
        if (!dom1 && !dom2) {
            ...
            //进行差异比较
            var text1 = $(dom1).text() || $(dom1).html();
            var value1 = replaceBr(text1);
            var text2 = $(dom2).text() || $(dom2).html();
            var value2 = replaceBr(text2);

            var result = _eq({value1: value1, value2: value2});
            $(dom1).html(result.value1);
            $(dom2).html(result.value2);
        }
    }
});